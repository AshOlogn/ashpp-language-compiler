...
let rec gen_three_address_expr ast temp label = 
    ...
    | EVar name -> 
        let new_temp = AddrVariable ("t" ^ (string_of_int temp)) in 
        let new_ins =  ThreeCopy (new_temp, AddrVariable name) in
    ...
    | EAssign (name, op, exp) -> 
        ...
        | OIden ->
            (* e.g. y = 5+3 *)
            let new_ins = ThreeCopy (AddrVariable name, addr) in
            (AddrVariable name, instructions @ [{label=None; instruction=new_ins;}], temp', label') 
        | _ -> 
            (* e.g. y *= 5+3 *)
            let new_temp = AddrVariable ("t" ^ (string_of_int temp')) in 
            let new_ins1 = {label=None; instruction=ThreeBinary (new_temp, AddrVariable name, op, addr)} in 
            let new_ins2 = {label=None; instruction=ThreeCopy (AddrVariable name, new_temp)} in 
            (AddrVariable name, instructions @ [new_ins1; new_ins2], temp'+1, label'))
    | _ -> (AddrVariable "dummy", [], 1, 1)
...
and gen_three_address_stat ast temp label = 
    ...
    | SDecl (_, name, expr) ->
        ...
        let new_ins = {label=None; instruction=ThreeCopy (AddrVariable name, addr);} in 
